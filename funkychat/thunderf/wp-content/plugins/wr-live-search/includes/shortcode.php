...
<form ' . ( $settings['id'] != '' ? 'id="' . esc_attr( $settings['id'] ) . '"' : '' ) . ' role="search" method="get" class="wrls-form ' . esc_attr( $settings['class'] ) . '" action="' . esc_url( home_url( '/' ) ) . '">';
	...
	$html .= '
		<input required data-max-results="' . (int) $settings['max_results'] . '" data-min-characters="' . (int) $settings['min_characters'] . '" data-search-in=' . "'" . json_encode( $settings['search_in'] ) . "'" . ' data-show-suggestion="' . ( ( $settings['show_suggestion'] === 'yes' || $settings['show_suggestion'] == 1 ) ? 1 : 0 ) . '" value="' . esc_attr( get_query_var( 's' ) ) . '" placeholder="' . esc_attr( $settings['placeholder'] ) . '" class="txt-livesearch suggestion-search" type="search" name="s" autocomplete="off">
	</div>';
