...
function yp_get_slider_markup($cssName, $name, $default = 'inherit', $decimals, $pxv, $pcv, $emv,$note = null)
{
    ...
    if(!defined("WTFV")){
	    if($cssName == 'width' || $cssName == 'height' || $cssName == 'opacity'){
	    	$notice_last = "<p class='yp-alert-warning yp-notice-last'>".$name." ".__('property not available for lite version.','yp')." <a target='_blank' href='http://webgep.co.za/livedesign/buy'>".__('Upgrade','yp')."?</a></p>";
	    }
    }
